...
public class MovementTech : MonoBehaviour
{
    ...
    void ExecuteWallJump()
    {
        ...
        // Combine Up, Away from wall, AND forward momentum!
        ...
        // Reset Dash and tag as Tech so you maintain landing speed
        canDash = true;
        dashCooldownTimer = 0f;
        jumpCooldownTimer = 0.2f;
        techActive = true;
        ...
        if (!isDashing)
        {
            ...
            if (!isWallrunning)
            {
                if (!isGrounded && currentHorizontal.magnitude > walkSpeed)
                {
                    if (isLettingGo)
                    {
                        // If you let go of WASD, keep drifting at your current high speed
                        targetVelocity = currentHorizontal;
                    }
                    else
                    {
                        // Use WASD to steer, but cap the target speed to what you already have!
